...
class Bug2Algorithm : public amp::BugAlgorithm {
    public:
        explicit Bug2Algorithm(bool leftTurning);   // constructor declaration
        amp::Path2D plan(const amp::Problem2D& problem) override;
    
    private:
        static constexpr double EPSILON = 0.002;   // collision buffer
        static constexpr double STEP_SIZE = 0.005; // step size towards goal
        static constexpr double DELTA_THETA = 0.03; // amount for robot to turn while perimeter following (in rad)
